...
import { ref } from 'vue';
const form = useForm({
    query_title: usePage().props.query.query_title,
    database_id: usePage().props.query.database_id,
    query: usePage().props.query.query,
    table_name:usePage().props.query.table_name,
    schema_name:usePage().props.query.schema_name,
    schedule:usePage().props.query.schedule,

});
const select_stmt = usePage().props.select_stmt
const updateQuery = () => {
    form.put(route('queries.update', usePage().props.query.id), {
        errorBag: 'updateQuery',
        preserveScroll: true,
    });
};
</script>
...
<template>
    <FormSection @submitted="updateQuery">
        <template #title>
            Query details
        </template>
